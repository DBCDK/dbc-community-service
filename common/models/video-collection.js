...
module.exports = function(VideoCollection) {
  VideoCollection.createResolution = function createVideoResolution(
    videoCollectionId,
    message,
    cb
  ) {
    ...
    message.outputs.forEach(videoOutput => {
      VideoCollection.app.models.resolution.create(
        ...
        function(err, resolutionObject) {
          if (err) {
            logger.error(
              'An error occurred during creation of video resolution',
              {error: err}
            );
            return cb(err);
          }
            ...
            VideoCollection.app.models.file.create(
              {
                ...
                url:
                  ...
                  '/download/' +
                  encodeURIComponent(videoOutput.key),
  ...
  VideoCollection.newVideoCollection = function newVideoCollection(data, next) {
    ...
    VideoCollection.create({}, function(error1, newVideoCollectionObject) {
      if (error1) {
        logger.error('An error occurred during creation of video collection', {
          error: error1
        });
        next(error1);
      } else {
        VideoCollection.app.models.resolution.create(
          ...
          function(error2, newResolutionObject) {
            if (error2) {
              logger.error(
                'An error occurred during creation of video collection',
                {error: error2}
              );
              next(error2);
            } else {
              VideoCollection.app.models.file.create(
                {
                  ...
                  url:
                    CONTAINERS_URL +
                    encodeURIComponent(data.container) +
                    '/download/' +
                    encodeURIComponent(data.videofile),
  ...
  VideoCollection.observe('before delete', function(ctx, next) {
    ...
    VideoCollection.find(
      ...
      (err, instances) => {
        if (err) {
          logger.error(
            'An error occurred during delete of a video collection',
            {error: err}
          );
        }
        ...
        instances.forEach(instance => {
          (instance.resolutions && Array.isArray(instance.resolutions)
