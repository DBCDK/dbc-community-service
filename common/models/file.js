          ...
          File.create(
            ...
            function(error, obj) {
              if (error) {
                ...
                logger.info(
                  'File was uploaded to S3, and relation was created'
                );
                cb(null, obj);
              }
            }
  ...
  File.upload = (ctx, options, container, cb) => {
    ...
    busboy.on('file', (fieldname, file, filename, encoding, mimetype) => {
      s3.upload(
        ...
        (err, data) => {
          if (err) {
            logger.error('An error occurred while uploading to amazon', {
              error: err
            });
            cb(err);
          } else {
            File.create(
              ...
              function(error, obj) {
                if (error) {
                  logger.error('An error occurred while creating file entry', {
                    error: error
                  });
                  cb(error);
                } else {
