...
module.exports = function(ImageCollection) {
  ...
  setTimeout(() => {
    ...
    amazonBuckets = ImageCollection.app.get('amazonConfig').buckets
      .imageBuckets;
  ...
  ImageCollection.createResolution = (
    fileBuffer,
    container,
    fileObj,
    size,
          ...
          resolutionModel.create(
            ...
            function(err2, resolutionObject) {
              if (err2) {
                ...
                fileModel.updateAll(
                  {id: fileObject.id},
                  {resolutionImageFileId: resolutionObject.id},
                  function(err3) {
                    if (err3) {
                      cb(err3);
                    } else {
  ...
  ImageCollection.upload = (ctx, options, container, cb) => {
    ...
    fileModel.upload(ctx, options, container, (err1, fileObj) => {
      if (err1) {
        logger.error('An error occurred during upload of imagecollection', {
          error: err1
        });
        cb(err1);
      } else {
        ...
        ImageCollection.create({}, function(err2, imageCollectionObject) {
          if (err2) {
            logger.error('An error occurred while creating image collection', {
              error: err2
            });
            cb(err2);
          } else {
            ...
            resolutionModel.create(
              ...
              function(err3, resolutionObject) {
                if (err3) {
                  logger.error('An error occurred while creating resolution', {
                    error: err3
                  });
                  cb(err3);
                } else {
                  fileModel.updateAll(
                    ...
                    function(err4) {
                      if (err4) {
                        logger.error(
                          'An error occurred during update of file',
                          {error: err4}
                        );
                        cb(err4);
                      } else {
                        // now it's time to find the object with relations and send it to the user
                        ImageCollection.findById(
                          imageCollectionObject.id,
                          {
                            include: [
                              {
                          ...
                          function(err5, imageCollectionInstance) {
                            if (err5) {
                              logger.error(
                                'An error occurred while finding image collection',
                                {error: err5}
                              );
                              cb(err5);
                            } else {
                              ...
                              // For the tests...
                              if (!imageQueue) {
                                imageQueue = ImageCollection.app.get(
                                  'imageQueue'
                                );
                              }
  ...
  ImageCollection.remoteMethod('upload', {
   ...
   * @param {Function} cb
   */
  ImageCollection.download = function downloadImageFromImageCollection(
    ctx,
    id,
    size,
    cb
    ...
    if (
      ...
      ImageCollection.findById(
        id,
        {include: [{relation: 'resolutions', scope: {include: ['image']}}]},
        function(err, imageCollection) {
          if (err) {
            cb(err);
          } else {
            ...
            if (resolutions.length > 0) {
              ...
              cb(
                null,
                Object.assign({correctSize}, resolution.image().toJSON())
              );
            } else {
  ...
  ImageCollection.remoteMethod('download', {
    description:
    ...
    accepts: [
      {arg: 'ctx', type: 'object', http: {source: 'context'}},
      {
        ...
        type: 'number',
        required: true,
        description: 'Id of the Image Collection.'
      },
      {
        arg: 'size',
        type: 'string',
        required: true,
        description:
          'Size of the image, either: original, small, medium or large.'
      }
    ],
    returns: {
      arg: 'image',
   ...
   */
  ImageCollection.resize = function resizeImageFromImageCollectionId(
    ctx,
    id,
    size,
    cb
    ...
    if (
      ...
      ImageCollection.findById(id, whereClause, function findImageToResize(
        err,
        imageCollection
      ) {
        if (err) {
  ...
  ImageCollection.remoteMethod('resize', {
    description: '',
    accepts: [
      {arg: 'ctx', type: 'object', http: {source: 'context'}},
      {
  ...
  ImageCollection.observe('before delete', function(ctx, next) {
    ImageCollection.find(
      {where: ctx.where, include: ['resolutions']},
      (err, instances) => {
