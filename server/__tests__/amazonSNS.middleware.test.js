...
describe('Test Amazon middlewares work as expected', () => {
  ...
  it('should confirm a valid SNS subscription request', done => {
    ...
    let Token =
      ''; // eslint-disable-line
        ...
        superagent
          ...
          .send(
            JSON.stringify({
              ...
              TopicArn,
              Subject:
                'Amazon Elastic Transcoder has finished transcoding job 1457366706277-el5ihc.',
              Message: JSON.stringify({
                // Double stringified cause that's how amazon does it.
                state: 'COMPLETED',
                version: '2012-09-25',
                ...
                input: {
                  key:
                    'uploads/1457366705388_25_1457346078061_70_Horse_Front_Flip!.mov'
                },
                outputs: [
                  {
                    id: '1',
                    ...
                    key:
                      '1457366705388_25_1457346078061_70_Horse_Front_Flip!.mp4',
                    thumbnailPattern:
                      '1457366705388_25_1457346078061_70_Horse_Front_Flip!_thumb_{count}',
                    status: 'Complete',
                    duration: 7,
                    width: 854,
                    height: 480
          ...
          .end((err, res) => {
            ...
            superagent
              .get(
                `${app.get('url')}api/Posts/${
                  postCreateResponse.body.id
                }?filter=${JSON.stringify({
                  include: [
                    {relation: 'video', scope: {include: ['resolutions']}}
                  ]
                })}`
              )
              .end((postGetError, postGetResponse) => {
                ...
                expect(Object.keys(postGetResponse.body.video)).toInclude(
                  'resolutions'
                );
                expect(
                  Array.isArray(postGetResponse.body.video.resolutions)
                ).toExist();
                expect(postGetResponse.body.video.resolutions.length).toEqual(
                  2
                );
                done();
              });
